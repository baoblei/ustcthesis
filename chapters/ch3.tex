...
\caption{补丁获取方法}
\label{alg:patch}
\KwIn{任意分辨率的图片输入 $\bm{X}$，输入图像 $\bm{X}$ 的宽度 $W$ 和高度 $H$，滑动窗口的步长 $s$，截取补丁块的滑动窗口大小 $p$}
\KwOut{提取的补丁块 $\mathbf{x}^{(i)}$ 和补丁位置掩码 $\bm{P}_i$}
\SetKwInOut{Input}{输入}
...
$\bm{P} \gets \mathbf{0}$ \quad \tcp{和图像 $\bm{X}$ 大小一致的补丁块位置掩码}
...
\begin{algorithm}
    \SetAlgoLined
    \KwIn{参考图像 $\bm{X}_0$ ，退化图像 $\bm{\tilde{X}}$，记录补丁位置的掩码 $\bm{P}$}
    \KwOut{训练后的模型参数 $\theta$}
...
\begin{algorithm}[ht]
    ...
    \For{$s = S,\ldots,1$}{
      ...
      $\bm{\hat{\epsilon}}_t \leftarrow \bm{\hat{\epsilon}}_t \oslash \mathbf{M}$  \tcp* {$\oslash$: 逐元素除法}
...
本章利用预训练模型在 RUIE \cite{RUIE} 数据集上进行了参数交叉的对比实验，RUIE是一个大规模的真实世界水下图像增强数据集，
与之前实验中用到的数据集不同的是RUIE的所有水下图像仅提供了从水下环境中采集的原始图像，而没有对应的参考图像。
